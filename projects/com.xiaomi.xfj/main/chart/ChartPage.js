...
export default class ChartPage extends React.Component {
    ...
    initChartData = () => {
        ...
        // 根据 getList 依次获取数据
        getList.forEach((key, index) => {
            console.log('initChartData key = ', key, index);
            setTimeout(() => {
                this.getChartData(key, 'week');
            }, index * 700);
        });
    }
        ...
        if (result.length === 0) {
            ...
            if (dt !== 'day') {
                ...
                result.forEach((item) => {
                    ...
                    // console.log('itemDate, nowDate', itemDate, nowDate, JSON.parse(item.value)[0]);
                    if (itemDate.getDate() === nowDate.getDate()) {
                        // 当天的数据
                        let value = JSON.parse(item.value)[0];
                        if (value) {
                            // 如果有0点的数据，则替换掉
                            if (itemDate.getHours() === 0 && itemDate.getMinutes() === 0) {
                                xData[0] = that.conversionTime(dt, item.time * 1000);
                                yData[0] = this.formatNumber(value);
                            } else {
                        ...
                        // 如果有前一天的数据，则保存按时间排序最后一条的数据，设置成当天的第一条数据0点数据
                        let yesterday = new Date();
                        yesterday.setDate(yesterday.getDate() - 1);
                        if (itemDate.getDate() === yesterday.getDate()) {
                            let value = JSON.parse(item.value)[0];
                            if (value) {
                                let time = that.conversionTime(dt, new Date(yesterday.getFullYear(), yesterday.getMonth(), yesterday.getDate() + 1, 0, 0, 0).getTime());
                                xData = [time];
                                yData = [this.formatNumber(value)];
                            }
                        }
                    }
                });
    ...
    getChartData = (key, dateType) => {
        ...
        if (dt === 'day') {
            dayNums = 2;
            startTime = startTime - 1 * 24 * 60 * 60;
    ...
    render() {
        // 缓存计算结果，避免在 render 中重复计算
        const tempProps = this.getTempProps();
        const humiProps = this.getHumiProps();
        const co2Props = this.getCo2Props();
        const pm25Props = this.getPm25Props();
        const vocProps = this.getVocProps();
        const cho2Props = this.getCho2Props();
                    ...
                    <View style={styles.container}>
                        ...
                        {this.state.hasIndoorTemp || this.state.hasOutdoorTemp ? (
                            <View style={styles.mt12}>
                                <ChartCom params={tempProps} changeDate={this.changeDate}></ChartCom>
                            </View>
                        ) : null}
                        {this.state.hasIndoorHumi || this.state.hasOutdoorHumi ? (
                            <View style={styles.mt12}>
                                <ChartCom params={humiProps} changeDate={this.changeDate}></ChartCom>
                            </View>
                        ) : null}
                        {this.state.hasCo2 ? (
                            <View style={styles.mt12}>
                                <ChartCom params={co2Props} changeDate={this.changeDate}></ChartCom>
                            </View>
                        ) : null}
                        {this.state.hasPm25 ? (
                            <View style={styles.mt12}>
                                <ChartCom params={pm25Props} changeDate={this.changeDate}></ChartCom>
                            </View>
                        ) : null}
                        {this.state.hasVoc ? (
                            <View style={styles.mt12}>
                                <ChartCom params={vocProps} changeDate={this.changeDate}></ChartCom>
                            </View>
                        ) : null}
                        {this.state.hasCho2 ? (
                            <View style={styles.mt12}>
                                <ChartCom params={cho2Props} changeDate={this.changeDate}></ChartCom>
                            </View>
                        ) : null}
                    </View>
                </ScrollView>
