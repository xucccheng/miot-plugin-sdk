...
import { API_LEVEL, Package, Host, Device, PackageEvent, DeviceEvent, Service, DarkMode } from 'miot';
...
import { faultCode } from '../resources/strings/en';
...
export default class MainPage extends React.Component {
  ...
  constructor(props) {
    ...
    handleTimerConfirm = (value) => {
      console.log('handleTimerConfirm', value);
      this.setState({ timerValue: value, timerVisible: false });
      let params = [
        { did: Device.deviceID, siid: 5, piid: 1, value: value }
      ];
    ...
    getBackgroundColor = () => {
      ...
      switch (mode) {
        ...
        case 2: // 换气 - 蓝色
          return ['#87CEEB', '#F6F6F6'];
        case 3: // 凉干燥 - 蓝色
          return ['#00CED1', '#F6F6F6'];
        case 4: // 凉干燥 - 蓝色
          return ['#FF9900', '#F6F6F6'];
        default:
          return ['#00CED1', '#F6F6F6'];
      }
    }
    ...
    callPhone = (phone) => {
      ...
      Linking.canOpenURL(url).then((supported) => {
        if (!supported) {
          Alert.alert('无法拨打电话', `设备不支持拨打 ${ phone }`);
          return;
        }
        Linking.openURL(url).catch((err) => {
          console.log('openURL error', err);
          Alert.alert('拨号失败', '请稍后重试');
        });
      }).catch((err) => {
        ...
        modeList: [
          {
            ...
            value: this.state.windDirection === 1,
            img: {
          ...
          },
          {
            ...
            value: this.state.windDirection === 0,
            img: {
        ...
        modeList: [
          {
            ...
            value: this.state.fanCategory === 2,
            img: {
          ...
          },
          {
            ...
            value: this.state.fanCategory === 1,
            img: {
    ...
    get PuStateProps() {
      console.log('PuStateProps', this.state.status);
      return {
        modeTxt: this.getPuModeText,
        gearTxt: this.getPuGearText,
        windDirectionTxt: this.getPuWindDirectionText,
        windTypeTxt: this.getPuWindTypeText,
    ...
    get getPuModeText() {
      let mode = this.state.mode;
      console.log('getPuModeText', mode);
      switch (mode) {
        case 0:
          return PluginStrings.standby;
        case 1:
    ...
    // 跳转到故障页面
    goToFaultPage = () => {
      this.props.navigation.navigate('FaultPage', { faultCode: this.state.fault });
    }
        ...
        <View style={styles.container}>
          ...
          <ScrollView style={styles.container} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContentContainer}>
            <View style={styles.statusSection}>
              <PuStateCom PuState={this.PuStateProps} ParticleColor={this.getBackgroundColor()[0]} goToFaultPage={this.goToFaultPage} />
            </View>
            {/* 故障如果是0 或者 4 */}
            {(this.state.fault === 0 || this.state.fault === 4) &&
                        <View>
                          ...
                          {!this.isStandbyOrHotDryOrHeating() && (
                            <View>
                              {/* 风量 */}
                              {this.shouldShowFanLevel() && <FuncCom params={this.funcThirdParams} onControlPress={this.handleFuncComPress} />}
                              {/* 风向 */}
                              {this.shouldShowWindDirection() && <FuncCom params={this.funcFourthParams} onControlPress={this.handleFuncComPress} />}
                              {/* 风种 */}
                              {this.shouldShowFanCategory() && <FuncCom params={this.funcFifthParams} onControlPress={this.handleFuncComPress} />}
            ...
            {/* 售后服务热线（已移出 ScrollView，固定底部） */}
          </ScrollView>

          <TimerModal
            visible={this.state.timerVisible}
            selectedValue={this.state.timerModeSet}
